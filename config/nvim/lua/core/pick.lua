	...
	--     require('mini.pick').start({
	  --       source = {
		...
		-- 	  func = function(item)
		  ...
		  --       },
		  --       preview = {
		    -- 	-- Usa la preview built-in che apre il buffer target
		    -- 	setup = function(item, ctx)
		      -- 	  if not item then return end
		      --
		      -- 	  -- Imposta il buffer della preview
		      -- 	  vim.api.nvim_win_set_buf(ctx.preview_win, item.buf_id)
		      --
		      -- 	  -- Metti il cursore sulla riga corrispondente
		      -- 	  vim.api.nvim_win_set_cursor(ctx.preview_win, { item.lnum, item.col - 1 })
		      --
		      -- 	  -- Centra la riga
		      -- 	  vim.cmd(string.format("normal! %dz", item.lnum))
		      -- 	end,
